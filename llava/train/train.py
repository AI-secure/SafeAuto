...
class ModelArguments:
    ...
    init_sigma: Optional[float] = field(default=-1)
    final_sigma: Optional[float] = field(default=-1)
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        try:
            ...
            if 'image' in sources[0]:
                ...
                video_files = self.list_data_dict[i]['video'] # A list 
                video_folder = self.data_args.video_folder
                processor = self.data_args.video_processor
                videos = []
...
def train():
    ...
    # Execute Here!
        ...
        # Hack here to use Driving Specific Training Prompt Header
        tokenizer.pad_token = tokenizer.unk_token
        # if model_args.version in conversation_lib.conv_templates:
            # conversation_lib.default_conversation = conversation_lib.conv_templates[model_args.version]
        conversation_lib.default_conversation = conversation_lib.conv_templates["driving"]
        # else:
    ...
    # Setup unified training configuration (sigma schedule + training schedule)
    train_dataset = data_module['train_dataset']
    dataset_size = len(train_dataset)
    
    # Calculate number of GPUs (for distributed training)
    num_gpus = max(1, training_args.world_size) if hasattr(training_args, 'world_size') else 1
    
    model.setup_training_configuration(
        # Sigma parameters
        init_sigma=model_args.init_sigma,
        final_sigma=model_args.final_sigma,
        tokenizer=tokenizer,
